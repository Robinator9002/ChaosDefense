...
// Sets the GTK version to 3 to avoid conflicts, because Electron 36+
// by default wants to use GTK 4 under GNOME, which can cause issues
// if other parts of the application or native modules expect GTK 2/3.
...
function createWindow() {
  const mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      // preload: path.join(__dirname, 'preload.js') // Include your preload script here if needed
    },
  });
...
// Some APIs can only be used after this event has occurred.
app.whenReady().then(() => {
  createWindow();
...
// You can add more application-specific main process logic here.
